...
El shell proporciona otra herramienta que resuelve estos dos problemas a la vez, llamada **tubería**.
Una vez más,
...
`@instructions`
Utiliza `cut` para seleccionar todos los nombres de dientes de la columna 2 del archivo delimitado por comas `seasonal/summer.csv`, y luego pasa el resultado a `grep`, con una coincidencia invertida, para excluir la línea de cabecera que contiene la palabra "Diente *".`cut` y `grep` se trataron en detalle en el capítulo 2, ejercicios 8 y 11 respectivamente.*
...
2. eliminar la línea de encabezamiento que contiene la palabra "Fecha"; y
