...
3. La variable que lleva la cuenta de qué cosa está procesando actualmente el bucle (en nuestro caso, `filetype`).
...
pero para **_cada_** archivo estacional por separado. Utiliza `file` como nombre de la variable de bucle, y recuerda recorrer la lista de archivos `seasonal/*.csv` _(en lugar de "estacional/invierno.csv" como en el ejemplo_).
