...
これはどこでも同じです。変数 `X` の値を取得するには `$X` と書く必要があります。これは、シェルが「X という名前のファイル」と「X という名前の変数の値」を区別できるようにするためです。
...
`@feedback`
- 正解です。このループでは `files` ではなく `$files` を使っているため、リストは「files」という単語で構成されます。
- 不正解です。このループでは `files` ではなく `$files` を使っているため、リストは `files` の展開結果ではなく「files」という単語で構成されます。
...
たとえば、`current.csv` と `last year.csv`（名前にスペースが含まれる）という2つのファイルがあるとして、次のように入力した場合：
...
これまでに見たループはすべて、体内に単一のコマンドまたはパイプラインを持っています。
しかしループには任意の数のコマンドを含めることができます。
貝殻にどこで終わり、次の場所が始まるかを伝えるために、
セミコロンで区切る必要があります:
...
前のループで`echo`と`head`コマンドの間のセミコロンを忘れたとします。
そのため、シェルに実行を促すと:
