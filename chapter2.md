...
title: データの操作
description: >-
  前の章で見たコマンドを使えば、ファイルシステム内で物事を移動できます。この章では、それらのファイル内のデータを扱う方法を学びます。使用するツールはかなりシンプルですが、しっかりした土台です。
lessons:
  - nb_of_exercises: 12
...
ファイル名を変更したり削除したりする前に、
内容を見ておくとよいでしょう。
これを行う最も簡単な方法は`cat`です。
これはファイルの内容を画面に表示するだけです。
（名前は「ものをつなぎ合わせる」という意味の「連結する」に由来する略称で、指定した名前のすべてのファイルを1つずつ順番に出力するため、この名前が付けられています。）
...
`+`フラグで使う番号の前に`-n`記号を付けると何が起こるのかを確認してください。
...
`@instructions`
一致を反転させて、`molar`に単語`seasonal/spring.csv`が含まれていないすべての行を見つけ、その行番号を表示します。
ファイル名や検索語「molar」など、すべてのフラグを*前に*置くのが良いスタイルとされています。
