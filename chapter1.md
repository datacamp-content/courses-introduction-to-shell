...
`@hint`
現在の場所より下にあるファイルやディレクトリへの相対パスを、よく作成できます
現在の場所の絶対パスを目的のものの絶対パスから引くことで。
...
`@hint`
`cd`は「ディレクトリを変更する」を意味し、相対パスは先頭に「/」が付かないことを覚えておいてください。
...
そのため、上記のコマンドを実行すると、あなたの論文は二度と戻ってきません。
...
このコマンドを使って、ホームディレクトリの下に `yearly` という新しいディレクトリを作成してください。
...
ホームディレクトリには `~` ショートカットを、2つ目のパスには絶対パスではなく相対パスを使うことをおすすめします。
