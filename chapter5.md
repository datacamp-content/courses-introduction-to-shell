...
これは、シェル（`bash` という名前のプログラムにすぎません）に対して、`headers.sh` ファイルに含まれるコマンドを実行するよう指示しています。これにより、コマンドを直接実行した場合と同じ出力が得られます。
...
`seasonal` 内のすべての CSV ファイルから最初の列を抽出するためのコマンドです。
...
シェルコマンドが入ったファイルは***シェルスクリプト**、または短く「スクリプト」と呼ばれることもあります。スクリプトの名前が必ずしも `.sh` で終わる必要はありませんが、このレッスンでは、どのファイルがスクリプトかを把握しやすくするためにこの規則を使います。

スクリプトにはパイプも含められます。
例えば、`all-dates.sh` に次の行が含まれている場合:
...
は季節データファイルから重複のない日付を抽出し、`dates.out` に保存します。
...
`@instructions`
Nanoでスクリプト`count-records.sh`を編集し、2つの`____`プレースホルダーを`$@`と`-l`（_文字_）でそれぞれ埋めます
これにより、1つ以上のファイル内の行数をカウントします。
ただし、それぞれの最初の行は除きます。
...
Nano では、「コピー＆ペースト」は、コピーしたい行に移動し、`CTRL` + `K` でその行を切り取り、次に `CTRL` + `U` を2回押してそのコピーを2つ貼り付けることで行います。
...
これは、コマンド引数を間違った場所に置いてしまうときによくある問題です。
