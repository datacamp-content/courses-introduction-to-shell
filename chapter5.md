...
- `Ctrl` + `O`: guarda el archivo ("O" significa "salida"). _También tendrás que pulsar Intro para confirmar el nombre del archivo._
...
Un archivo lleno de comandos del shell se llama**\*script** del shell,
...
con `$@` y `-l` _(la letra_) respectivamente para que cuente el número de líneas de uno o varios archivos,
